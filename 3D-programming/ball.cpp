...
Ball::Ball(RenderWindow *Owner, int n)
...
void Ball::update(float deltaTime)
{
    ...
    pos = pos + Vec3(0.f, 0.f, speed);
    getTransform().setPosition(pos);
    speed = speed - speed/100;

    if(getTransform().getPosition().getZ() > 1.5f|| getTransform().getPosition().getZ() < -1.5f)
    {
        speed = -speed;
    }
}

void Ball::makeTriangle(const Vec3 &v1, const Vec3 &v2, const Vec3 &v3)
{
